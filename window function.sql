...
-- Display the avg claim amount	for patients in each region
...
select region, smoker from insurance_data
where smoker = 'yes'
group by region;

-- 6. What is the average claim amount for patients who are both diabetic and smokers?

select diabetic,smoker,
avg(claim)from insurance_data
...
select patientid,bmi,smoker from insurance_data
where smoker ='yes'and bmi > (select avg(bmi) from insurance_data
where smoker ='yes');
...
select * from (select *,avg(claim) over(partition by region) as avg_claim from insurance_data) as window_QUERY

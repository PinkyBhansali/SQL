SELECT * FROM data_thinkers.insurance_data;
...
select region, count(PatientID) from insurance_data
where smoker = 'Yes'
group by region;
...
select PatientID, gender, bmi from insurance_data
...
-- SECOND APPROCH
select * from (select *, avg(claim) over(partition by region) as avg_claim
